...
if(isset($_POST['submit'])) {
    ...
    if($inputPassword ==$inputPassword2)
    {
        ...
        $message = 'Hello, your registration finished!';
        $headers = 'From: http://i388250.hera.fhict.nl' . "\r\n" .
            'Reply-To: http://i388250.hera.fhict.nl' . "\r\n" .
            'X-Mailer: PHP/' . phpversion();
        mail($to, $subject, $message, $headers);

        $q= "SELECT COUNT(inputEmail) AS mail FROM accounts WHERE inputEmail = :inputEmail";
        $query = $odb->prepare($q);
        $query->execute(array(":inputEmail" => $inputEmail));
